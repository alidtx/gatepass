...
use Modules\User\Http\Requests\UserSourceRequest;
...
class UserSourceController extends Controller
{
    ...
    {
        // $this->middleware('permission:user-source-view-permiession', ['only' => ['index','show']]);
        // $this->middleware('permission:user-source-create-permission', ['only' => ['create', 'store']]);
        // $this->middleware('permission:user-source-edit-permission', ['only' => ['edit', 'update']]);
        // $this->middleware('permission:user-source-delete-permission', ['only' => ['destroy']]);
    }
    ...
    {
        try {
            $locations = UserSource::with('createdByUser')
                        ->active()
                        ->filter($request->all())
                        ->orderBy('id', 'desc')
                        ->sortable();
     ...
     * @return Renderable
     */
    public function store(UserSourceRequest $request)
    {
        try {
            $data = new UserSource();
    ...
    {
        try {
            $data = UserSource::with('createdByUser')->find($id);
     ...
     * @return Renderable
     */
    public function update(UserSourceRequest $request, $id)
    {
        try {
            $data = UserSource::find($id);
            $data->name = $request->name;
            $data->status = $request->status;
            $data->save();
